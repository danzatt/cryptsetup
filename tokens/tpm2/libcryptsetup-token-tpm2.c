...
const static crypt_token_arg_item args[] = {
	...
	{ TCTI_ARG,	"Select TCTI in format <tcti>:<tcti arg>, e.g. device:/dev/tpm0",               CRYPT_ARG_STRING, &args[6] },
	{ FORCE_REMOVE_ARG,	"Force remove the TPM token metadata from LUKS header, even if the TPM device is not present.",                                      CRYPT_ARG_BOOL,  &args[7] },
	/* inherited from cryptsetup core args */
	{ "key-size",	NULL,                                      CRYPT_ARG_UINT32, &args[8] },
	{ "token-id",	NULL,                                      CRYPT_ARG_INT32,  &args[9] },
	{ "key-slot",	NULL,                                      CRYPT_ARG_INT32,  &args[10] },
